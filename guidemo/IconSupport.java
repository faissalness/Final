...
public class IconSupport {
    ...
    private final ArrayList<BufferedImage> iconImages = new ArrayList<BufferedImage>();
    ...
    /**
     * Create the "Stamper" menu from actions that are defined later in this class.
     */

    public JMenu makeStamperMenu() {
...
//		menu.add(actions.get(actions.size()-1));
        ...
        for (int i = 0; i < iconNames.length - 1; i++) {
            menu.add(new SelectIconAction(iconNames[i], i));
        }

        menu.addSeparator();
        menu.add(new NoIconAction());
...
//	public class iconMenuAction extends AbstractAction {}
