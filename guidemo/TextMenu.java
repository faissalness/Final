...
public class TextMenu extends JMenu {
    ...
    private JRadioButtonMenuItem justifyText;
    JRadioButtonMenuItem[] justLable = {new JRadioButtonMenuItem("Left"), new JRadioButtonMenuItem("Center"), new JRadioButtonMenuItem("Right")};
    ...
    public TextMenu(DrawPanel owner) {
        ...
        lineSpacing.addActionListener(e -> {

            double currentSpacing = panel.getTextItem().getLineHeightMultiplier();
            String s = JOptionPane.showInputDialog(panel,
                    "Enter amount to multiply the default line spacing with: ",
                    currentSpacing);
            if (s != null && s.trim().length() > 0) {

                try {
                    double newSpacing = Double.parseDouble(s.trim());
                    panel.getTextItem().setLineHeightMultiplier(Math.abs(newSpacing));
                    panel.repaint();
                } catch (Exception ex) {

                    JOptionPane.showMessageDialog(panel, s + " isn't a legal input.\n"
        ...
        /**Create a new JMenu named "Justify." To this menu, you will add three JRadioButtonMenuItems named "Left",
         *  "Right", and "Center". To make the radio buttons into an actual group, create a ButtonGroup object, and
         *  add each radio button to the button group. Add the JMenu to the menu that is created by the constructor of
         *  the TextMenu class. You might want to use a makeJustifyMenu method, similar to (but simpler than) the
         *  makeFontNameSubmenu method that is already defined in TextMenu.*/
    ...
    public void setDefaults() {
        ...
        justLable[0].setSelected(true);
    }

    /**
     * Make and return the Justify" sub-menu
     */
    ...
    private JMenu makeJustifyMenu() {
        ...
        AbstractAction action = new AbstractAction("Justify") {
            @Override
            public void actionPerformed(ActionEvent e) {

                if (e.getActionCommand().equalsIgnoreCase(justLable[0].getActionCommand())) {

                    panel.getTextItem().setJustify(TextItem.LEFT);
                    panel.repaint();
                } else if (e.getActionCommand().equalsIgnoreCase(justLable[1].getActionCommand())) {
        ...
        for (JRadioButtonMenuItem jRadioButtonMenuItem : justLable) {

            jRadioButtonMenuItem.addActionListener(action);
            subMenu.add(jRadioButtonMenuItem);
            group.add(jRadioButtonMenuItem);
        }
        group.setSelected(justLable[0].getModel(), true);
